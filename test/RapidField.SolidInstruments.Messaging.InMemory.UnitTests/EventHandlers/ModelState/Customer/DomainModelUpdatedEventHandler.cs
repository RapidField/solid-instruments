...
namespace RapidField.SolidInstruments.Messaging.InMemory.UnitTests.EventHandlers.ModelState.Customer
{
    /// <summary>
    /// Processes a single <see cref="DomainModelEvent" />.
    /// </summary>
    internal class DomainModelUpdatedEventHandler : DomainModelUpdatedEventHandler<DomainModel, DomainModelEvent>
    {
        /// <summary>
        ...
        /// </exception>
        public DomainModelUpdatedEventHandler(ICommandMediator mediator)
            : base(mediator)
        {
            return;
        }
        ...
        {
            var existingModel = SimulatedServiceState.Customers.Where(entity => entity.Identifier == model.Identifier).FirstOrDefault();
            ...
            SimulatedServiceState.Customers.Remove(existingModel);
            SimulatedServiceState.Customers.Add(model);
        }
    }
}