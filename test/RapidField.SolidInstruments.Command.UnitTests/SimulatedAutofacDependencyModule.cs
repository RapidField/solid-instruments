...
namespace RapidField.SolidInstruments.Command.UnitTests
{
    ...
    internal sealed class SimulatedAutofacDependencyModule : AutofacDependencyModule
    {
        ...
        {
            configurator.RegisterType<SimulatedCommandHandler>().As<ICommandHandler<SimulatedCommand>>().InstancePerDependency();
            configurator.RegisterType<SimulatedCommandWithResultHandler>().As<ICommandHandler<SimulatedCommandWithResult>>().InstancePerDependency();
        }
    }
}