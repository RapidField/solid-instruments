...
namespace RapidField.SolidInstruments.Example.Domain.AccessControl.EventHandlers.ModelState.User
{
    ...
    public sealed class DomainModelDeletedEventHandler : DomainModelDeletedEventHandler<DomainModel, DomainModelEvent>
    {
        ...
        /// </param>
        protected override void Process(DomainModel model, IEnumerable<String> labels, Guid correlationIdentifier, ICommandMediator mediator, IConcurrencyControlToken controlToken) => Console.WriteLine($"A user was deleted (identifier: {model?.Identifier.ToSerializedString()}).");
    }
}