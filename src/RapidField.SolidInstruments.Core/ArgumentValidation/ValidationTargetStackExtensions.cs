...
{
    /// <summary>
    /// Extends the <see cref="ValidationTarget{TArgument}" /> class with validation features for <see cref="Stack{T}" /> arguments.
    /// </summary>
    public static class ValidationTargetStackExtensions
    {
        /// <summary>
        ...
        /// </exception>
        [DebuggerHidden]
        public static ValidationResult<Stack<T>> IsNullOrEmpty<T>(this ValidationTarget<Stack<T>> target, String targetParameterName) => target.RejectIfIsNullOrEmpty<Stack<T>, T>(targetParameterName);
    }
}