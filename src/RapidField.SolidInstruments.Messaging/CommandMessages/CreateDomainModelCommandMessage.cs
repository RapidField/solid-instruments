...
{
    /// <summary>
    /// Represents a message that contains a command to create an object that models a domain construct and which can be described
    /// by an <see cref="IEvent" />.
    /// </summary>
    /// <remarks>
    /// <see cref="CreateDomainModelCommandMessage{TModel, TCommand, TEvent, TEventMessage}" /> is a derivative of
    ...
    /// </typeparam>
    [DataContract]
    public abstract class CreateDomainModelCommandMessage<TModel, TCommand, TEvent, TEventMessage> : DomainModelCommandMessage<TModel, TCommand, TEvent, TEventMessage>, ICreateDomainModelCommandMessage<TModel, TCommand>
        where TModel : class, IDomainModel
        where TEvent : DomainModelEvent<TModel>, IDomainModelCreatedEvent<TModel>, new()
        where TCommand : CreateDomainModelReportableCommand<TModel, TEvent>, new()
        where TEventMessage : DomainModelEventMessage<TModel, TEvent>, IDomainModelCreatedEventMessage<TModel, TEvent>, new()
    {
        /// <summary>
        /// Initializes a new instance of the
        /// <see cref="CreateDomainModelCommandMessage{TModel, TCommand, TEvent, TEventMessage}" /> class.
        /// </summary>
        ...
        /// </param>
        protected CreateDomainModelCommandMessage(Boolean isReported)
            : base(isReported)
        {
            return;
        }
        ...
        /// <paramref name="commandObject" /> is <see langword="null" />.
        /// </exception>
        protected CreateDomainModelCommandMessage(TCommand commandObject, Boolean isReported)
        ...
        /// <see cref="CreateDomainModelCommandMessage{TModel, TCommand, TEvent, TEventMessage}" />.
        /// </summary>
        ...
        protected override void EnrichEventMessage(TEventMessage associatedEventMessage)
        {
            try
            {
                associatedEventMessage.Event.Classification = DomainModelEventClassification.Created;
            }
            finally
            {
