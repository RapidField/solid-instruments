...
namespace RapidField.SolidInstruments.Messaging.InMemory
{
    ...
    public sealed class InMemoryListeningFacade : MessageListeningFacade<IMessagingEntitySendClient, IMessagingEntityReceiveClient, PrimitiveMessage, InMemoryTransmittingFacade>
    {
        ...
        /// </returns>
        protected sealed override Task RouteToDeadLetterQueueAsync<TMessage>(PrimitiveMessage adaptedMessage, TMessage message, IEnumerable<String> pathLabels, MessagingEntityType entityType) => Task.CompletedTask; // In-memory listeners do not support dead lettering.
    }
}